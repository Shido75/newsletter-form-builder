...
export default function Subscribe() {
  ...
  const [colorTheme, setColorTheme] = useState<ColorTheme>("classic"); // Default to classic
  ...
  const getBackgroundClasses = () => {
    switch (colorTheme) {
      ...
      case "warm":
        return "from-orange-50 to-red-50"; // Use warm-toned background colors
      case "mono":
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center bg-gradient-to-br ${getBackgroundClasses()} p-4 relative overflow-hidden transition-colors duration-500`}>
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className={`absolute top-20 left-10 w-64 h-64 bg-${colorTheme}-primary/10 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse`}></div>
        <div className={`absolute bottom-20 right-10 w-80 h-80 bg-${colorTheme}-secondary/10 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse`} style={{ animationDelay: '1s' }}></div>
      </div>
